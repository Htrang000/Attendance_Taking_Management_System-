        ...
        <div>
            ...
            <c:if test="${requestScope.saList != null}">
                <div class="statistics">
                    ...
                    Percent absent: ${requestScope.percent}%<br>
                </div>
                <c:set var="numOfRows" value="${requestScope.saList.size() div 10}" />   
                <c:set var="list" value="${requestScope.saList}"/>
                <c:forEach begin="1" end="${numOfRows}" varStatus="a">
                    <c:if test="${a.index eq 1}">
                        <table border="1px">
                            <tr>
                                <c:forEach  begin="0" end="9" varStatus="loop">
                                    <th>Lesson ${list[loop.index].lesson.sessionNo}</th>
                                    </c:forEach>
                            </tr>
                            <tr>
                                <c:forEach  begin="0" end="9" varStatus="loop">
                                    <c:set var="sa" value="${list[loop.index]}" />
                                    <td>
                                        <div>
                    ...
                    <c:if test="${a.index ne 1}">
                        <table border="1px">
                            <tr>
                                <c:forEach  begin="10" end="${list.size()-1}" varStatus="loop">
                                    <th>Lesson ${list[loop.index].lesson.sessionNo}</th>
                                    </c:forEach>
                            </tr>
                            <tr>
